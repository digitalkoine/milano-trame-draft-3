...
map._tdPlayer = tdPlayer;
  ...
  // Current year box: create ONCE, then update the text on timeload.
  // The previous implementation added the control on every tick, causing stacked boxes.
  var currentTime = L.control({ position: 'topright' });
  currentTime.onAdd = function () {
    this._div = L.DomUtil.create('div', 'yearbox');
    this._div.innerHTML = '<h1>—</h1>';
    return this._div;
  };
  ...
  map.timeDimension.on('timeload', function () {
    ...
    if (currentTime._div) currentTime._div.innerHTML = '<h1>' + year + '</h1>';
  });
